...
const int LENGTH = 8; // defining the length of the floor
int floor [LENGTH] [LENGTH] = {0}; // array for values on floor appearently

void displayFloor();

int chooseRobotMove(int curRow, int curCol);
int rowChange[] = {-1, -1, -1, 0, 0, 1, 1, 1}; //now the robo has 8 moves :)
int colChange[] = {0, -1, 1, -1, 1, -1, 0, 1}; //it can move one direction in each direction
...
int main() {
    ...
    int chooseRow, chooseCol; //used to store next move
    while(true) {
